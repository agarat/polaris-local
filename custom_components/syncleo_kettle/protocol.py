...
class Message:
    ...
    def from_encrypted(buf: bytes, inkey: bytes, outkey: bytes) -> Message:
        ...
        elif head.type == FrameType.PING:
            _logger.debug("PING frame received, responding with ACK")
            return AckMessage(head.seq)
...
class UDPConnection(threading.Thread, ConnectionStatusListener):
    ...
    def run(self):
        ...
        while not self.interrupted:
            ...
            if sock is None or sock.fileno() == -1:
                ...
                try:
                    ...
                    if consecutive_errors >= max_consecutive_errors:
                        self._logger.error("Too many consecutive socket errors, stopping connection")
                        break
                    time.sleep(1)
                    continue
            ...
            if wm:
                ...
                if encrypted:
                    ...
                    try:
                        ...
                        self._logger.warning(f"Failed to send message, socket error: {exc}")
                        sock = None  # Force socket recreation
                        continue
            ...
            try:
                ...
                self._logger.warning(f"Socket error in receive: {exc}")
                consecutive_errors += 1
                if consecutive_errors >= max_consecutive_errors:
                    self._logger.error("Too many consecutive receive errors, recreating socket")
                    sock = None
                else:
                    time.sleep(0.1)
            except Exception as exc:
                self._logger.error(f"Unexpected error in receive: {exc}")
                consecutive_errors += 1
                if consecutive_errors >= max_consecutive_errors:
                    self._logger.error("Too many consecutive errors, stopping connection")
                    break
                time.sleep(0.1)
    ...
    def is_running(self) -> bool:
        """Check if connection thread is running and healthy."""
        return (self.is_alive() and 
                not self.interrupted and 
                hasattr(self, 'status') and 
                self.status in [ConnectionStatus.CONNECTED, ConnectionStatus.CONNECTING])
    ...
    def force_reconnect(self):
        """Force reconnection by stopping the thread."""
        self.interrupted = True
